...
import { createRecomendacaoAction } from "@/lib/actions/visitas";
...
export default async function NovaRecomendacaoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <PageHeader title="Nova recomendação" backHref={`/visitas/${id}`} />
      <Card>
        <CardContent>
          <RecomendacaoForm areas={areas ?? []} action={createRecomendacaoAction.bind(null, id)} />
        </CardContent>
      </Card>
    </div>
  );
